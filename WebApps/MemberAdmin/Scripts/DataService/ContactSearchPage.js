...
function User(data) {
    ...
    self.assocTypes = new Array();
    for (var i = 0; i < data.types.length; i++) {
        for (var j = 0; j < assocTypes.length; j++) {
            if (data.types[i] == assocTypes[j].id) {
                self.assocTypes.push(assocTypes[j]);
                break;
            }
        }
    }
        ...
        $.ajax({
            ...
            success: function (content) {
                if (content.hasDetails && content.details.hasPhoto) {
                    content.details.photoUrl = appRoot + 'Account/UserPhoto?id=' + self.data.ID;
                }
                self.more(content);
                callback(true);
            },
...
function UserPage() {
    ...
    self.GetPageItems = function (s, callback) {
        ...
        $.ajax({
            url: appRoot + "Contacts/GetMembers",
            type: "POST",
            dataType: "json",
                ...
                s.GetNextSorterOps(function (ok) {
                    if (ok) {
                        ...
                        if (s.CurrentSorters().Options.length == 2 && s.CurrentSorters().Options[0].IsExternal) {
                            s.CurrentSorters().Options[0].ImgSrc = "glyphicon glyphicon-chevron-up"; 
                            s.CurrentSorters().Options[1].ImgSrc = "glyphicon glyphicon-chevron-down";
                        }
...
function selectUser(data, event) {
    for (var i = 0; i < userSet.CurrentPage().Items().length; i++) {
