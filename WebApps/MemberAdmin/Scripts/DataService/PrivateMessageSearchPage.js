...
function ShortMessage(data) {
    ...
    self.fromIconUrl = ko.computed(function () {
        if (self.from.icon) {
            return appRoot + 'Account/GetMemberIcon?id=' + self.from.id;
        } else {
            return "";
        }
    });
    ...
    if (typeof data.replies != 'undefined' && data.replies.length > 0) {
        for (var i = 0; i < data.replies.length; i++) {
            self.Replies.push(new ShortMessage(data.replies[i]));
        }
    }
    self.LoadDetails = function (callback) {
        ...
        $.ajax({
            ...
            success: function (content) {
                ...
                self.root = new ShortMessage(data);
                callback(true);
            },
...
function ShortMessagePage(data) {
    var self = this;
    var self = this;
        ...
        $.ajax({
            ...
            success: function (content) {
                for (var i = 0; i < content.length; i++) {
                    ...
                    self.Items.push(new ShortMessage(msg));
                }
...
function ShortMessagePageBlock(idx0, data) {
    var self = this;
    self.BlockIndex = 0;
...
function ShortMessageSet(dataServiceUrl) {
    var self = this;
    ...
    self.GetSetInfo = function () {
        $.ajax({
            ...
            dataType: "json",
            contentType: "application/json; charset=utf-8",
            data: JSON.stringify({ sourceId: dataSourceId, set: JSON.stringify({ set: setName, setFilter: self.SetFilter }) }),
            beforeSend: function () {
            },
            success: function (content) {
        ...
        $.ajax({
            ...
            success: function (content) {
                ...
                if (data.Pages.length == 0) {
                    ...
                    var pb = new ShortMessagePageBlock(idx0, data);
...
function getQueryExpr() {
    var sorters = [];
    var filters = [];
    for (var i = 0; i < msgSet.SorterPath().length; i++)
        sorters.push(msgSet.SorterPath()[i]);
    for (var i = 0; i < msgSet.FilterPath().length; i++)
        filters.push(msgSet.FilterPath()[i]);
...
function showlist(e) {
    if (!queryCompleted)
        return;
    var qexpr = getQueryExpr();
    if (msgSet.IsQueryStateChanged()) {
        msgSet.ResetPageState();
    }
    msgSet.NextPageBlock(qexpr, null, function (ok, ch) {
        if (ch && msgSet.CurrentPage() != null && !(typeof msgSet.CurrentPage().Items === 'undefined')) {
            msgSet.CurrentPage().Items.removeAll();
        }
        if (ok) {
            msgSet.IsQueryInitialized(true);
            if (ch && msgSet.PageBlocks().length > 0 && msgSet.PageBlocks()[0].Pages().length > 0) {
                loadpage(0);
            }
        } else {
...
function prevPageBlock() {
    ...
    var idx = msgSet.CurrBlockIndex();
    if (idx > 0) {
        msgSet.CurrBlockIndex(idx - 1);
        msgSet.PagesWindow.removeAll();
        var ipage = -1;
        for (var i = 0; i < msgSet.PageBlocks()[idx - 1].Pages().length; i++) {
            var p = msgSet.PageBlocks()[idx - 1].Pages()[i];
            msgSet.PagesWindow.push(p);
            if (p.IsPageSelected()) {
                ipage = p.Index_();
            }
        }
...
function nextPageBlock() {
    ...
    var idx = msgSet.CurrBlockIndex();
    if (idx < msgSet.PageBlocks().length - 1) {
        msgSet.CurrBlockIndex(idx + 1);
        msgSet.PagesWindow.removeAll();
        var ipage = -1;
        for (var i = 0; i < msgSet.PageBlocks()[idx + 1].Pages().length; i++) {
        ...
        idx = msgSet.PageBlocks().length - 1;
        var b = msgSet.PageBlocks()[idx];
        if (!b.IsLastBlock()) {
            msgSet.CurrBlockIndex(idx + 1);
            var p = b.LastPage();
            if (p == null) {
            ...
            var qexpr = getQueryExpr();
            msgSet.NextPageBlock(qexpr, p.LastItem(), function (ok, ch) {
                if (ok) {
                    if (msgSet.PageBlocks().length > 0 && msgSet.PageBlocks()[idx + 1].Pages().length > 0) {
                        loadpage(msgSet.PageBlocks()[idx + 1].Pages()[0].Index_());
                    }
                }
            });
        }
...
function loadpage(index) {
    ...
    var p = null;
    var p0 = null;
    var blk = msgSet.PageBlocks()[msgSet.CurrBlockIndex()];
    for (var i = 0; i < blk.Pages().length; i++) {
        var _p = blk.Pages()[i];
        if (_p.Index_() == index) {
            p = _p;
    ...
    if (p != null) {
        if (!p.IsDataLoaded()) {
            p.GetPageItems(msgSet, function (ok) {
                if (ok) {
                    updateCurrPage(p, p0);
...
function selectMessage(data, event) {
    for (var i = 0; i < msgSet.CurrentPage().Items().length; i++) {
        var e = msgSet.CurrentPage().Items()[i];
        if (e.IsEntitySelected() && e != data) {
            e.IsEntitySelected(false);
        }
    }
    msgSet.CurrentSelected(data);
...
function updateEntityDetails(msg) {
    msg.LoadDetails(function (ok) {
        if (ok) {
            msgSet.CurrentPage().CurrentItem(msg.root);
            var pos = $(window).scrollTop();
            var docheight = $(document).height();
            var winheight = $(window).height();
            if (pos + winheight < docheight) {
                if (scrolling) {
                    clearTimeout(scrollFunc);
                    scrolling = false;
                }
                scrollFunc = setTimeout(slowScrollBottom, 200);
            } else {
                clearTimeout(scrollFunc);
                scrolling = false;
            }
...
function slowScrollBottom() {
    ...
    if (pos + winheight < docheight) {
        ...
        scrollFunc = setTimeout(slowScrollBottom, 1);
    } else {
        clearTimeout(scrollFunc);
        scrolling = false;
    }
